...
rho1 = zeros(11);
rho2 = zeros(11);
E_nooffload1 = zeros(11);
E_nooffload2 = zeros(11);
E_offload1 = zeros(11);
E_offload2 = zeros(11);
...
for n=1:maxlambda
    ...
    latency_nooffload_p = [latency_nooffload_p E_nooffload1];
...
for i=1:11
    ...
    for j=1:11
        ...
        if varphi_1 == 0
            ...
            E_offload1(i,j) = (varphi_1*lambda(1)*b*b + (1-varphi_2)*lambda(2)*b*b)/((1-varphi_1*lambda(1)*b)*(1-varphi_2*lambda(2)*b-(1-varphi_1)*lambda(1)*b)) + b + 2.0*10^-3;
            E_nooffload2(i,j) = (varphi_1*lambda(1)*b*b + (1-varphi_2)*lambda(2)*b*b)/((1-varphi_1*lambda(1)*b)) + b;
        end
        if varphi_2 == 0
            E_offload2(i,j) = 0;
            E_nooffload1(i,j) = 1/(1/b - lambda1(i,j));
        else 
            E_offload2(i,j) = ((1-varphi_1)*lambda(1)*b*b + varphi_2*lambda(2)*b*b)/((1-varphi_2*lambda(2)*b)*(1-varphi_1*lambda(1)*b-(1-varphi_2)*lambda(2)*b)) + b + 2.0*10^-3;
            E_nooffload1(i,j) = ((1-varphi_1)*lambda(1)*b*b + varphi_2*lambda(2)*b*b)/((1-varphi_2*lambda(2)*b)) + b;

        end

        utility1(i,j) = (-lambda_offload1(i,j) + lambda_offload2(i,j) + 0.1*lambda1(i,j))*b + 100*(lambda_offload1(i,j)*b*(d_rq-E_offload1(i,j)) + lambda_nooffload1(i,j)*b*(d_rq-E_nooffload1(i,j)));
        utility2(i,j) = (-lambda_offload2(i,j) + lambda_offload1(i,j) + 0.1*lambda2(i,j))*b + 100*(lambda_offload2(i,j)*b*(d_rq-E_offload2(i,j)) + lambda_nooffload2(i,j)*b*(d_rq-E_nooffload2(i,j)));
    end
end

utility1_latency_offload = d_rq - E_offload1;
utility1_latency_nooffload = d_rq - E_nooffload1;
utility2_latency_offload = d_rq - E_offload2;
utility2_latency_nooffload = d_rq - E_nooffload2;

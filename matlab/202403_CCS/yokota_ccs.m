...
for i=1:maxlambda
    ...
    rho1_p = [rho1_p (1-0.1*i1-0.1)*lambda_i(1)+(0.1*i2-0.1)*lambda_i(2)-(0.1*i1-0.1)*lambda_i(1)-(1-0.1*i2-0.1)*lambda_i(2)];
    rho2_p = [rho2_p (1-0.1*i11-0.1)*lambda_i(1)+(0.1*i22-0.1)*lambda_i(2)-(0.1*i11-0.1)*lambda_i(1)-(1-0.1*i22-0.1)*lambda_i(2)];
...
hold on
grid on
scatter(lambda_p, varphi1_p(1:maxlambda),100, 'filled', 'blue');
scatter(lambda_p, varphi2_p(1:maxlambda),100, 'x', 'red');
scatter(lambda_p, varphi11_p(1:maxlambda),100, 'filled', 'green');
scatter(lambda_p, varphi22_p(1:maxlambda),100, '+', 'magenta');
ylim([0 1]);
...
hold on
grid on
scatter(lambda_p, rho1_p(1:maxlambda),100, 'filled', 'blue');
scatter(lambda_p, rho2_p(1:maxlambda),100, 'filled', 'red');
xlabel('Arrival rate of \lambda_{12}'); ylabel('Load balancing');
l1 = legend('with priority', 'without priority');
l1.FontSize = 10;
hold off